...
function crawlPage() {
  ...
  const cardData = [...cardNodes].map((card) => {
    ...
    const cardHead = cardDetails.querySelector('.infoCol').innerText.trim().split('|');

    // Required Values
    const number = cardHead[0].trim();

    const _set = cardDetails.querySelector('.getInfo').innerText.replace('Card Set(s)', '').trim();
    const set = getSetNameAndID(_set);

    const rarity = cardHead[1].replace('CARD', '').trim();

    const color = cardDetails.querySelector('.color').innerText.replace('Color', '');

    const type = cardDetails.querySelector('.feature').innerText.replace('Type', '');

    const category = cardHead[2].trim();

    const cost = cardDetails.querySelector('.cost').innerText.replace('Cost', '');
    ...
    const _effect = cardDetails.querySelector('.text').innerText.replace('Effect', '');
    const effect = _effect === '-' ? undefined : _effect;

    const _power = cardDetails.querySelector('.power').innerText.replace('Power', '');
    const power = _power === '-' ? undefined : _power;

    const _counter = cardDetails.querySelector('.counter').innerText.replace('Counter', '');
    const counter = _counter === '-' ? undefined : _counter;

    const _trigger = cardDetails.querySelector('.trigger');
    const trigger = _trigger ? _trigger.innerText.replace('Trigger', '').trim() : undefined;
